...
/**
 * WebGL fluid-splash cursor trail, scoped to its containing element only.
 * The container must be `position: relative` (or similar) and sized to the
 * area the effect should cover — it fills that area via `position: absolute; inset: 0`
 * and only reacts to pointer movement while the cursor is over that area.
 */
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Only react to pointer activity while the cursor is over this canvas's
    // own screen area — the effect must stay confined to its section.
    function withinCanvasBounds(clientX: number, clientY: number) {
    ...
    function handleMouseMove(e: globalThis.MouseEvent) {
      if (!withinCanvasBounds(e.clientX, e.clientY)) return
      const pointer = pointers[0]
      const { x: posX, y: posY } = relativePos(e.clientX, e.clientY)
      if (!started) {
        started = true
        const color = generateColor()
        updateFrame()
    ...
    function handleTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        if (!withinCanvasBounds(touches[i].clientX, touches[i].clientY)) continue
        const { x: posX, y: posY } = relativePos(touches[i].clientX, touches[i].clientY)
        if (!started) {
          started = true
          updateFrame()
        }
